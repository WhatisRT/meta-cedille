...
instance
  ...
  PureTerm-TermLike = record { Var = Var-P ; SortC = Sort-P ; _⟪$⟫_ = App-P }
...
{-# TERMINATING #-}
byUniformFoldPure : (𝕀 → 𝕀 → PureTerm) → PureTerm → PureTerm
byUniformFoldPure f = helper 0
  where
...
modifyIndicesPure n =
  byUniformFoldPure λ k x → if x <𝕀 k then Var-P (Bound x) else BoundVar (pred𝕀 (x +𝕀 n))

weakenPureBy : 𝕀 → PureTerm → PureTerm
weakenPureBy i = modifyIndicesPure (suc𝕀 i)

strengthenPure : PureTerm → PureTerm
strengthenPure = modifyIndicesPure 0

-- substitute the first unbound variable in t with t'
{-# TERMINATING #-}
substPure : PureTerm → PureTerm → PureTerm
substPure t t' = strengthenPure $ byUniformFoldPure
  (λ k x → if k ≣ x then weakenPureBy (suc𝕀 k) t' else Var-P (Bound x)) t
...
instance
  ...
  AnnTerm-TermLike = record { Var = Var-A ; SortC = Sort-A ; _⟪$⟫_ = App-A }
...
{-# TERMINATING #-}
byUniformFold : (𝕀 → 𝕀 → AnnTerm) → AnnTerm → AnnTerm
byUniformFold f = helper 0
  where
...
modifyIndices n = byUniformFold λ k x → if x <𝕀 k then Var-A (Bound x) else BoundVar (pred𝕀 (x +𝕀 n))

weakenBy : 𝕀 → AnnTerm → AnnTerm
weakenBy i = modifyIndices (suc𝕀 i)

strengthen : AnnTerm → AnnTerm
...
subst t t' = strengthen $ byUniformFold
  (λ k x → if k ≣ x then weakenBy (suc𝕀 k) t' else Var-A (Bound x)) t
...
Erase (LamE-A _ t t₁)  = strengthenPure (Erase t₁)
