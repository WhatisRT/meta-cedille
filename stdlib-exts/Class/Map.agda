...
record MapClass (M : Set -> Set -> Set) : Set₁ where
  field
    insert : ∀ {A B} {{_ : Eq A}} -> A -> B -> M A B -> M A B
    remove : ∀ {A B} {{_ : Eq A}} -> A -> M A B -> M A B
    lookup : ∀ {A B} {{_ : Eq A}} -> A -> M A B -> Maybe B
    mapSnd : ∀ {A B C} -> (B -> C) -> M A B -> M A C
    emptyMap : ∀ {A B} -> M A B

open MapClass {{...}} public

mapFromList : ∀ {A B} {M} {{_ : MapClass M}} {{_ : Eq B}} -> (A -> B) -> List A -> M B (List A)
